<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Contained Antimatter</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Contained Antimatter](images/refined_antimatter.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>legendary</td></tr>
<tr><td><b>Size</b></td><td>2</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>2,500 cr</td></tr>
</table>

> Magnetically confined antimatter for extreme energy applications.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Generate Antimatter](refine_antimatter.md) | 1 | 60 ticks | Advanced Crafting 7, Ore Refinement 9, Radioactive Handling 7 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Build Singularity Core](craft_singularity_core.md) | 2 | [Singularity Core](comp_singularity_core.md) |
