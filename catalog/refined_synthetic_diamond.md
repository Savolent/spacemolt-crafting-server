<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Synthetic Diamond</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Synthetic Diamond](images/refined_synthetic_diamond.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>rare</td></tr>
<tr><td><b>Size</b></td><td>1</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>300 cr</td></tr>
</table>

> Lab-grown diamond for cutting tools and heat sinks.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Compress Synthetic Diamond](refine_synthetic_diamond.md) | 1 | 8 ticks | Ore Refinement 5 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Assemble Quantum Matrix](assemble_quantum_matrix.md) | 1 | [Refined Quantum Matrix](refined_quantum_matrix.md) |
