<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Crystalline Lattice</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Crystalline Lattice](images/refined_crystalline_lattice.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>exotic</td></tr>
<tr><td><b>Size</b></td><td>1</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>550 cr</td></tr>
</table>

> Perfectly ordered crystal structure for energy systems.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Grow Crystalline Lattice](grow_crystalline_lattice.md) | 1 | 20 ticks | Advanced Crafting 4, Ore Refinement 6 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Synthesize Metamaterial](synthesize_metamaterial.md) | 2 | [Metamaterial Panel](refined_metamaterial.md) |
