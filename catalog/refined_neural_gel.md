<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Neural Interface Gel</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Neural Interface Gel](images/refined_neural_gel.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>rare</td></tr>
<tr><td><b>Size</b></td><td>1</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>400 cr</td></tr>
</table>

> Biocompatible medium for brain-computer interfaces.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Synthesize Neural Gel](process_neural_gel.md) | 1 | 5 ticks | Advanced Crafting 2 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Manufacture Neural EM Charges](craft_em_neural.md) | 1 | [Neural Disruption Charge Pack](em_neural.md) |
| [Culture Repair Nanites](craft_repair_nanite_container.md) | 2 | [Repair Nanite Container](comp_repair_nanites.md) |
