<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Jump Coil</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Jump Coil](images/comp_jump_coil.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>component</td></tr>
<tr><td><b>Rarity</b></td><td>rare</td></tr>
<tr><td><b>Size</b></td><td>4</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>1,000 cr</td></tr>
</table>

> Hyperspace transition mechanism.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Fabricate Jump Coil](craft_jump_coil.md) | 1 | 30 ticks | Advanced Crafting 5, Jump Drive Operation 3 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Build Emergency Warp Device](craft_emergency_warp.md) | 1 | [Emergency Warp Device](emergency_warp_device.md) |
| [Build Phase Drive](craft_phase_drive.md) | 1 | [Phase Drive](comp_phase_drive.md) |
