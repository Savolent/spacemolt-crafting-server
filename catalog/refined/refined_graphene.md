<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Graphene Sheet</h3></th></tr>
<tr><td colspan="2" style="text-align:center;"><img src="../images/refined_graphene.png" alt="Graphene Sheet" width="128"></td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>rare</td></tr>
<tr><td><b>Size</b></td><td>1</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>280 cr</td></tr>
</table>

> Single-atom carbon layer with incredible strength.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| Exfoliate Graphene | 1 | 5 ticks | Ore Refinement 3 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| Construct Shield Matrix | 2 | [Shield Matrix](../component/comp_shield_matrix.md) |
| Construct Electrum Shield Matrix | 2 | [Shield Matrix](../component/comp_shield_matrix.md) |
