<!-- Auto-generated from crafting.db — do not edit manually -->

<table>
<tr><th colspan="2" style="text-align:center;"><h3>Bioluminescent Culture</h3></th></tr>
<tr><td colspan="2" style="text-align:center;">

![Bioluminescent Culture](images/refined_biolume_culture.png)

</td></tr>
<tr><th colspan="2" style="text-align:center;">General</th></tr>
<tr><td><b>Category</b></td><td>refined</td></tr>
<tr><td><b>Rarity</b></td><td>rare</td></tr>
<tr><td><b>Size</b></td><td>1</td></tr>
<tr><td><b>Stackable</b></td><td>Yes</td></tr>
<tr><td><b>Tradeable</b></td><td>Yes</td></tr>
<tr><th colspan="2" style="text-align:center;">Market</th></tr>
<tr><td><b>Base Value</b></td><td>150 cr</td></tr>
</table>

> Cultivated light-producing organisms for sensor technology.

## Crafting

### Produced By

| Recipe | Qty | Crafting Time | Skills Required |
|--------|-----|---------------|-----------------|
| [Culture Bioluminescent Cells](process_bioluminescent_cells.md) | 2 | 10 ticks | Biological Processing 4 |

### Used In

| Recipe | Qty | Produces |
|--------|-----|----------|
| [Synthesize Focus Enhancer](synthesize_focus_enhancer.md) | 1 | [Focus Stim](stim_focus.md) |
